...
用法：
    python -m src.trading_system.factors.extensions.tushare_daily_task_X

执行后将：
1. 注册 daily_open, daily_high, daily_low, daily_close,
   daily_pre_close, daily_change, daily_pct_chg, daily_vol, daily_amount 共 9 个因子定义
2. 从 TuShare daily 接口抓取近一年数据
3. 将因子值存入数据库
"""
...
# ── 用户配置：修改此处的股票代码 ──────────────────────
#
SYMBOLS = ["X"]   # <-- 将 "X" 替换为实际 ts_code，如 "600519.SH"
#
# ─────────────────────────────────────────────────────
